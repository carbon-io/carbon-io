...
Both techniques rely on Object Server :doc:`authentication <authentication>`.
...
For example, a ``/contacts`` endpoint meant to retrieve the all the ``Contacts`` for the currently authenticated user might be written as:
...
For example, a ``/users/:userId/contacts`` endpoint meant to retrieve the all the ``Contacts`` the currently authenticated user has permission to see might be written as:
